...
class ResourceSpaceAdminMCPServer {
  ...
  private tools: ReturnType<typeof createAdminTools>;

  constructor() {
    this.server = new Server(
      {
        name: 'resourcespace-admin-mcp',
        version: '1.0.0',
      },
      {
        capabilities: {
    ...
    this.tools = createAdminTools(this.client);
  ...
  async run() {
    ...
    console.error(`ResourceSpace Admin MCP server running on stdio (${this.tools.length} tools)`);
  }
}

// Start the server
const server = new ResourceSpaceAdminMCPServer();
