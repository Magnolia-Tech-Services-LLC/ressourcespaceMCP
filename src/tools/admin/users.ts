...
export function createAdminUserTools(client: ResourceSpaceClient): MCPTool[] {
  return [
    // Consolidated user CRUD
    {
      name: 'user',
      description: `Manage user accounts - perform get, create, update, or delete operations.
...
- delete: Delete user (requires user_id)`,
      inputSchema: z.object({
        ...
        user_id: z.union([z.string(), z.number()]).optional().describe('User ID (required for get, update, delete)'),
        ...
        usergroup: z.number().optional().describe('User group ID (required for create)'),
    ...
    },
    {
      ...
      inputSchema: z.object({
        usergroup: z.number().optional().describe('Filter by user group ID'),
      }),
      handler: async (args: { usergroup?: number }) => {
        const params: (string | number)[] = args.usergroup !== undefined ? [args.usergroup] : [];
        const users = await client.call('get_users', ...params);
        return { users };
      },
    },
    // Consolidated usergroup CRUD
    {
      name: 'usergroup',
      description: `Manage user groups - perform get, create, update, or delete operations.
...
- save: Save user preferences (requires preferences object, optional user_id)`,
    ...
    },
    {
      ...
      inputSchema: z.object({
        user_id: z.union([z.string(), z.number()]).describe('User ID to approve'),
      }),
      handler: async (args: { user_id: string | number }) => {
