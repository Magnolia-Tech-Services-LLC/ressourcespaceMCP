...
import { createIIIFTools_All } from './tools/iiif/index.js';
import { ResourceSpaceError } from './types/resourcespace.js';

class ResourceSpaceIIIFMCPServer {
  private server: Server;
  private client: ResourceSpaceClient;
  private tools: ReturnType<typeof createIIIFTools_All>;

  constructor() {
    this.server = new Server(
      {
        name: 'resourcespace-iiif-mcp',
        version: '1.0.0',
      },
      {
        capabilities: {
    ...
    // Initialize IIIF tools
    this.tools = createIIIFTools_All(this.client);
  ...
  async run() {
    ...
    console.error(`ResourceSpace IIIF MCP server running on stdio (${this.tools.length} tools)`);
  }
}

// Start the server
const server = new ResourceSpaceIIIFMCPServer();
